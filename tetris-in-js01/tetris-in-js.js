...
function generatePiece() {
    ...
    newPiece.rowPos=0;
    newPiece.colPos=4;
    for (var i = 0; i < newPiece.children.length; i++) {
        block = newPiece.children[i]
        block.rowPos = newPiece.rowPos + layout[i][0];
        block.colPos = newPiece.colPos + layout[i][1];
...
function moveRight(piece) {
   ...
   for (var i = 0; i < piece.children.length; i++) {
        block = piece.children[i]
        if (block.colPos>=9||occupationMatrix[block.rowPos][block.colPos+1]){
            atright=1
        }
    }
    if (atright==0){
...
function pieceMoveDown(piece) {
   ...
   for (var i = 0; i < piece.children.length; i++) {
        block = piece.children[i]
        if (block.rowPos>=19||occupationMatrix[block.rowPos+1][block.colPos]){
            atdown=1;
        }
    }
    if (atdown==0){
...
function fallToBottom(piece) {
    ...
    *************************/
    ...
    while (pieceMoveDown(document.getElementById("currentPiece"))){
    }
    show();
    /************************
        YOUR CODE ENDS
...
function rotateClock(piece) {
    ...
    *************************/
    var canrotate = 1;
    rowCenter=piece.rowPos;
    colCenter=piece.colPos;
    console.log("rowcenter:",rowCenter,"colCenter")
    for (var i = 0; i < piece.children.length; i++) {
        block = piece.children[i]
        newcolpos=(-block.rowPos+rowCenter)+colCenter;
        newrowpos=(block.colPos-colCenter)+rowCenter;
        if (newcolpos>9||newcolpos<0||newrowpos<0||newrowpos>19||occupationMatrix[newrowpos][newcolpos]){
            canrotate=0;
        }
    }
    if (canrotate==1){
        for (var i = 0; i < piece.children.length; i++) {
            ...
            newcolpos=(-block.rowPos+rowCenter)+colCenter;
            newrowpos=(block.colPos-colCenter)+rowCenter;
            console.log("row:",newrowpos,"col:",newcolpos);
            block.rowPos=newrowpos;
            block.colPos=newcolpos;
        }
        setPosition(piece);
        console.log("正在转");
        return true;
